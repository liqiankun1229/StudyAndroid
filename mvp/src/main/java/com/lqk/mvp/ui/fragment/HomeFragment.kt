...
class HomeFragment : Fragment(), MyScrollView.OnScrollMove {
...
//        view.findViewById<RecyclerView>(R.id.rcv_home_item).adapter = homeAdapter
    ...
    private fun initData() {
        ...
        serviceList = initList(mutableListOf(
    ...
    override fun onChange(l: Int, t: Int, oldl: Int, oldt: Int) {
        ...
        var top = head.height - (ll_search.height / 2) - t
        // 获取状态栏高度
        var height = 0
        ...
        if (top > head.height - (ll_search.height / 2)) {
            top = head.height - (ll_search.height / 2)
        } else if (top < height) {
            top = height
        }
        if (top == height) {
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                ...
                activity!!.window.navigationBarColor = resources.getColor(R.color.colorPrimary)
                activity!!.window.statusBarColor = resources.getColor(R.color.colorPrimary)
            }
            ll_search_background.background = resources.getDrawable(R.drawable.bg_hide_search)
            ll_search_background.gravity = Gravity.CENTER
            ll_search_background.setPadding(0, 5, 0, 5)
            tv_search.setCompoundDrawablesWithIntrinsicBounds(resources.getDrawable(R.drawable.search_white),
                    null, null, null)
            tv_search.textColor = Color.WHITE
            tv_search.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12f)
            ...
            if (Build.VERSION.SDK_INT >= 21) {
                ...
                decorView.systemUiVisibility = option
                activity!!.window.navigationBarColor = resources.getColor(R.color.colorPrimary)
                activity!!.window.statusBarColor = resources.getColor(R.color.colorTransParent)
            }
            ll_search_background.background = resources.getDrawable(R.drawable.bg_search)
            ll_search_background.gravity = Gravity.CENTER_VERTICAL
            ll_search_background.setPadding(10, 10, 10, 10)
            tv_search.setCompoundDrawablesWithIntrinsicBounds(resources.getDrawable(R.drawable.search),
                    null, null, null)
            tv_search.textColor = Color.parseColor("#999999")
            tv_search.setTextSize(TypedValue.COMPLEX_UNIT_SP, 18f)
        }

        val layoutParams: FrameLayout.LayoutParams = ll_search.layoutParams as FrameLayout.LayoutParams
        layoutParams.setMargins(0, top, 0, 0)
        ll_search.background = resources.getDrawable(R.color.colorPrimary)
        // 修改透明度
        val alpha = 255 - (top / (head.height.toDouble() - (ll_search.height / 2).toDouble()) * 255)
        ll_search.background.alpha = alpha.toInt()
