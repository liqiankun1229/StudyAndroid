...
import com.lqk.butter.zxing.core.oned.Code93Reader;
...
public class Code93Writer extends OneDimensionalCodeWriter {
  ...
  public boolean[] encode(String contents) {
    ...
    //start character (*)
    toIntArray(com.lqk.butter.zxing.core.oned.Code93Reader.CHARACTER_ENCODINGS[47], widths);

    boolean[] result = new boolean[codeWidth];
    int pos = appendPattern(result, 0, widths);

    for (int i = 0; i < length; i++) {
      int indexInString = com.lqk.butter.zxing.core.oned.Code93Reader.ALPHABET_STRING.indexOf(contents.charAt(i));
      toIntArray(com.lqk.butter.zxing.core.oned.Code93Reader.CHARACTER_ENCODINGS[indexInString], widths);
      pos += appendPattern(result, pos, widths);
    }

    //add two checksums
    int check1 = computeChecksumIndex(contents, 20);
    toIntArray(com.lqk.butter.zxing.core.oned.Code93Reader.CHARACTER_ENCODINGS[check1], widths);
    pos += appendPattern(result, pos, widths);

    //append the contents to reflect the first checksum added
    contents += com.lqk.butter.zxing.core.oned.Code93Reader.ALPHABET_STRING.charAt(check1);

    int check2 = computeChecksumIndex(contents, 15);
    toIntArray(com.lqk.butter.zxing.core.oned.Code93Reader.CHARACTER_ENCODINGS[check2], widths);
    pos += appendPattern(result, pos, widths);
